...
TEMPLATE_TABLE = 'summary_table2.tex'
...
if __name__ == '__main__':
    ...
    #rule_dd_files = get_all_csv_files(INPUTS_PATH, 'csv')
    rule_dd_files = []
    abagraph_files = get_all_csv_files(ABAGRAPH_PATH, 'csv')

    files = rule_dd_files + abagraph_files
    ...
    for s, df in dfs.items():
        ...
        for f_key, fun in functions_dict.items():
            answer = str(fun(df))
            template_string = template_string.replace(f'{setup}_{f_key}', answer)



    caption = f'normal' if not NON_TRIVIAL else f'normal non trivial'
    ...
    name = f'summary_table_out_{"nt" if NON_TRIVIAL else ""}.tex'
